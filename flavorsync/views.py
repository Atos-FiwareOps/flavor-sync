...
@app.route("/v1/infrastructures/<infrastructure_name>", methods=['DELETE'])
def unregister_infrastructure(infrastructure_name):
	manager = FlavorSynchronizer()
	manager.unregister_infrastructure(infrastructure_name)
...
def publish_or_promote_flavor(flavor_id):
	...
	flavor = Flavor.deserialize(content_type, request.data)
	manager = FlavorSynchronizer()
	
	if not flavor.nodes:
		modified_flavor = manager.publish_or_promote_flavor(flavor_id, flavor)
	else:
		modified_flavor = manager.add_flavor_to_infrastructure(flavor_id, flavor)
